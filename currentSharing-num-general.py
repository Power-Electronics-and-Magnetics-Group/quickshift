...
def current_sharing(N, pl, sl, b, f, l, r, Ip = 1):
    '''
...
def traverse_layer_list(node, N, d, bOverL, R, array):
    ...
    if (type(node) == int or len(node)==1):
        ...
        array.append(faraday_equation(a,b,N,d,bOverL))
        traverse_layer_list(node[1], N, d, bOverL, R, array)
        traverse_layer_list(node[2], N, d, bOverL, R, array)
        return array
    else: return 0
...
print(current_sharing(8, ['s',2,['s', 3, ['s', 4, 7]]], ['p', 1, ['p', 5, ['p', 6, 8]]], 80,23,48,89,90))
